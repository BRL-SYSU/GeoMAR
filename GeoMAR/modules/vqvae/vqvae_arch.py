...
import GeoMAR.distributed as dist_fn
...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self, n_e, e_dim, beta, entropy_loss_ratio=0.1, entropy_temperature=1.0):
    ...
    def forward(self, z):
        ...
        # compute standard loss for embedding
        commitment_loss = torch.mean((z_q.detach()-z)**2)
        embedding_loss = torch.mean((z_q - z.detach()) ** 2)
        vq_loss = commitment_loss * self.beta + embedding_loss

        # Add the entropy-loss calculation.
        entropy_loss = 0.0
        if self.training and self.entropy_loss_ratio > 0:
            # Negate distances to obtain similarity scores.
            neg_distances = -d / self.entropy_temperature
            
            # Compute probabilities and log probabilities.
            probs = F.softmax(neg_distances, dim=1)
            log_probs = F.log_softmax(neg_distances + 1e-5, dim=1)
            
            # Compute the mean probability of selecting each codebook entry.
            avg_probs = torch.mean(probs, dim=0)
            
            # Compute mean entropy to encourage diverse codebook usage.
            avg_entropy = -torch.sum(avg_probs * torch.log(avg_probs + 1e-5))
            
            # Compute sample entropy to encourage confident predictions at each position.
            sample_entropy = -torch.mean(torch.sum(probs * log_probs, dim=1))
            
            # Final entropy loss: sample entropy minus mean entropy.
            entropy_loss = (sample_entropy - avg_entropy) * self.entropy_loss_ratio
            
            # Print debugging information.
            if torch.rand(1)[0] < 0.01:  # Print information with 1% probability.
                # Compute codebook utilization.
                unique_indices = torch.unique(min_encoding_indices)
                usage_percent = 100.0 * unique_indices.numel() / self.n_e
                print(f"Codebook utilization: {unique_indices.numel()}/{self.n_e} ({usage_percent:.2f}%)")
                print(f"Entropy loss: {entropy_loss.item():.4f}, mean entropy: {avg_entropy.item():.4f}, sample entropy: {sample_entropy.item():.4f}")

        # Combine all loss terms.
        loss = vq_loss + entropy_loss
        ...
        # Return the extended information tuple.
        return z_q, loss, (perplexity, min_encodings, min_encoding_indices, d), self.embedding.weight

    def get_codebook_entry(self, indices, shape): # Decode.
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, temb):
        try:
            ...
            # Print tensor shapes.
            #print(f"ResnetBlock shapes - x: {x.shape}, h: {h.shape}")
            ...
            # Final check.
            if x.shape != h.shape:
                print(f"WARNING: Shape mismatch! x:{x.shape}, h:{h.shape}")
                
            return x+h
        except Exception as e:
            print(f"Error in ResnetBlock: {e}")
...
class MultiHeadEncoder(nn.Module):
    def __init__(self, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks=2,
        ...
        for i_level in range(self.num_resolutions):
            ...
            down.block = block
            down.attn = attn
            if i_level != self.num_resolutions-1: # Do not downsample at the final level.
    ...
    def forward(self, x):
        ...
        hs = {} # Dictionary for storing intermediate results.
        # timestep embedding
        temb = None

        # downsampling
        h = self.conv_in(x) # Convert the 512x512x3 input image to 512x512xch.
        hs['in'] = h
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
            ...
            if i_level != self.num_resolutions-1: # Do not downsample at the final level; the final output is 16x16x512.
                # hs.append(h)
        ...
        h = self.conv_out(h)
        # hs.append(h)
        hs['out'] = h  # Final output with z_channels channels.
...
class VQVAEGANMERGE(nn.Module):
    ...
    def encode(self, x):
        ...
        quant, emb_loss, info, dictionary = self.Fixed_quantize(h)  ####LQ encoder +origin HQ codebook
        # return quant, emb_loss, info, hs
        print("LQ encoder + origin HQ codebook")
        return quant, emb_loss, info, hs, h, dictionary
