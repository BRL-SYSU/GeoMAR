...
from main_GeoMAR import instantiate_from_config, DataModuleFromConfig
    ...
    for idx, cropped_face in enumerate(face_helper.cropped_faces):
        ...
        try:
            with torch.no_grad():
                ...
                _, xrec,_,_ = model.maskgit_generate(x = cropped_face_t,   filenames=[basename], gt = None)
                restored_face = tensor2img(xrec.squeeze(0), rgb2bgr=True, min_max=(-1, 1))

        except RuntimeError as error:
            print(f'\tFailed inference for GeoMAR: {error}.')
...
if __name__ == "__main__":
    ...
    if hasattr(model, 'vqvae'):
        print(f"VQVAE class name: {type(model.vqvae).__name__}")
    ...
    print(f"test_path (raw): {opt.test_path}")
    print(f"test_path (abs): {os.path.abspath(opt.test_path)}")
    print(f"Found files: {len(img_list)}")
    if len(img_list) > 0:
        print(img_list[:20])

    print('Results are in the <{}> folder.'.format(outdir))
    model.eval()  
    model.alignmodel.set_eval_mode(True)
